...
export default function DashboardPage() {
  ...
  return (
    <main id="pdf-report-content" className={`min-h-screen flex flex-col relative overflow-x-hidden mx-auto border-x border-paper-outline/30 transition-all duration-700 ease-in-out px-4 md:px-0 w-full ${hasCalculated ? 'max-w-[1400px]' : 'md:max-w-4xl'}`}>

      <header className="flex justify-between items-center p-4 border-b border-paper-outline/30 bg-background/50 backdrop-blur-sm z-10 sticky top-0">
      ...
      <div className="bg-paper-secondary py-2 px-4 flex flex-col gap-1 border-b border-paper-outline/30">
        ...
        <div className="flex justify-between items-center">
          ...
          <div className="flex items-center gap-2 text-xs">
            ...
            <div className="w-2 h-2 rounded-full bg-paper-primary "></div>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 p-4 pb-24 flex flex-col gap-6">
        ...
        <div className={`grid grid-cols-1 w-full items-start gap-6 transition-all duration-700 ${hasCalculated ? 'xl:grid-cols-2' : ''}`}>
          ...
          <TuiSection title={t('businessMetrics')} variant="left">
            ...
            <div className="mt-8 pt-4 mt-auto border-t border-paper-outline/30 border-dashed">
              <TuiButton
                onClick={() => {
            ...
            {hasCalculated && (
              <>
                ...
                <div className={`${isCalculating || isEstimating ? 'hidden' : 'block'} space-y-6`}>
                  ...
                  <div className="space-y-6 bg-card p-4 -mx-4 border border-paper-outline/20">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <TuiDataBox
                        label={t('avgQPS')}
                        value={formatNumber(projections.avgQPS, { minimumFractionDigits: 1, maximumFractionDigits: 2 })}
                        infoText={t('descAvgQPS')}
                      />
                      <TuiDataBox
                        label={t('peakQPS')}
                        value={formatNumber(projections.peakQPS, { minimumFractionDigits: 1, maximumFractionDigits: 2 })}
                        infoText={t('descPeakQPS')}
                        subValue={(activeMetrics.PeakFactor || 1) > 1.0 ? `+${formatNumber(projections.peakQPS - normalProjections.peakQPS, { maximumFractionDigits: 1 })} vs Normal` : undefined}
                      />
                      <TuiDataBox
                        label={t('readQPS')}
                        value={formatNumber(projections.readQPS, { minimumFractionDigits: 1, maximumFractionDigits: 2 })}
                        infoText={t('descReadQPS')}
                        largeValue={false}
                        subValue={(activeMetrics.PeakFactor || 1) > 1.0 ? `Peak: ~${formatNumber(projections.readQPS * (activeMetrics.PeakFactor || 1), { maximumFractionDigits: 1 })}` : undefined}
                      />
                      <TuiDataBox
                        label={t('writeQPS')}
                        value={formatNumber(projections.writeQPS, { minimumFractionDigits: 1, maximumFractionDigits: 2 })}
                        infoText={t('descWriteQPS')}
                        largeValue={false}
                        subValue={(activeMetrics.PeakFactor || 1) > 1.0 ? `Peak: ~${formatNumber(projections.writeQPS * (activeMetrics.PeakFactor || 1), { maximumFractionDigits: 1 })}` : undefined}
                      />
                    </div>

                    <div className="border border-paper-outline/30 border-dashed my-6"></div>
                      ...
                      <TuiDataBox
                        ...
                        value={
                          <InfoTooltip content={getStorageDetails(projections.totalEgressGB)}>
                            <span className="cursor-help border-b border-paper-primary/30 border-dashed hover:border-paper-primary transition-colors">{formatDataSize(projections.totalEgressGB)}</span>
                          </InfoTooltip>
                        }
                      ...
                      <TuiDataBox
                        ...
                        value={
                          <InfoTooltip content={getStorageDetails(projections.totalStorageGB)}>
                            <span className="cursor-help border-b border-paper-primary/30 border-dashed hover:border-paper-primary transition-colors">{formatDataSize(projections.totalStorageGB)}</span>
                          </InfoTooltip>
                        }
      ...
      <footer className="mt-auto bg-card border-t border-paper-primary/30 p-8 flex flex-col md:flex-row justify-between items-center text-[10px] font-bold text-paper-primary/60 tracking-widest uppercase gap-4 md:gap-0">
        <div className="flex flex-col md:flex-row items-center gap-2 md:gap-6">
          <span>© 2026 INFRALYZER - VERSION 1.0.0</span>
          <Link href="/design-system" className="hover:text-paper-primary transition-colors flex items-center gap-1 group">
            <span className="text-paper-primary ">[</span>
            <span className="group-hover:text-paper-primary transition-colors">DESIGN SYSTEM</span>
            <span className="text-paper-primary ">]</span>
          </Link>
        </div>
        <div>
          <a
            ...
            className="hover:text-paper-primary transition-colors flex items-center gap-2"
          >
            <span className="text-paper-primary ">{'>'}</span> DEVELOPED BY MATHEUSJFA
          </a>
        </div>
      </footer>
    </main>
