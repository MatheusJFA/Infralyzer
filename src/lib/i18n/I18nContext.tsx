  ...
  const getStorageDetails = (gb: number) => {
    ...
    return (
      <div className="flex flex-col gap-1 text-left w-full mt-1">
        <div className="flex justify-between items-center gap-6 border-b border-paper-primary/10 pb-2 mb-1">
          <span className="opacity-60 text-[11px] uppercase tracking-widest font-bold">Megabytes</span>
          <span className="font-bold text-paper-primary text-sm tracking-tight">{formatNumber(mb, { maximumFractionDigits: 2 })} MB</span>
        </div>
        <div className="flex justify-between items-center gap-6 border-b border-paper-primary/10 pb-2 mb-1">
          <span className="opacity-60 text-[11px] uppercase tracking-widest font-bold">Gigabytes</span>
          <span className="font-bold text-paper-primary text-sm tracking-tight">{formatNumber(gb, { maximumFractionDigits: 2 })} GB</span>
        </div>
        <div className="flex justify-between items-center gap-6 pt-1">
          <span className="opacity-60 text-[11px] uppercase tracking-widest font-bold">Terabytes</span>
          <span className="font-bold text-paper-primary text-sm tracking-tight">{formatNumber(tb, { maximumFractionDigits: 2 })} TB</span>
        </div>
      </div>
    );
  };
